...
const labelKeyPrefixRegex = /^([a-z0-9A-Z]([a-z0-9A-Z-.]*[a-z0-9A-Z])?)+$/;
const labelKeyNameRegex = /^([a-z0-9A-Z]([a-z0-9A-Z-_.]*[a-z0-9A-Z])?)$/;
const labelValueRegex = labelKeyNameRegex;
export function isValidLabel(type, value) {
  if (type === 'key') {
    ...
    if (parts.length === 2) {
      ...
      if (!labelKeyPrefixRegex.test(prefix)) {
        return false;
      }
    }
    ...
    return labelKeyNameRegex.test(name);
  }
  if (type === 'value') {
    if (value === '') {
      // allow empty values
    ...
    return labelValueRegex.test(value);
  }
  return false;
}
