...
function getFormattedResources({
  ...
  return resources.map(stepAction => ({
    ...
    name: (
      <Link
        ...
        to={urls.rawCRD.byNamespace({
          ...
          type: 'stepactions',
          name: stepAction.metadata.name
        })}
        title={stepAction.metadata.name}
      >
    ...
    namespace: stepAction.metadata.namespace,
    createdTime: (
      <FormattedDate date={stepAction.metadata.creationTimestamp} relative />
    ),
    actions: !isReadOnly ? (
      <Button
        ...
        kind="ghost"
        onClick={() =>
          openDeleteModal([{ id: stepAction.metadata.uid }], () => {})
        }
...
function StepActions() {
    ...
    if (error) {
      return {
        ...
        title: intl.formatMessage({
          id: 'dashboard.stepActions.errorLoading',
          defaultMessage: 'Error loading StepActions'
        })
      };
    }
  ...
  function handleDeleteStepAction(stepAction) {
    deleteStepAction({
      name: stepAction.metadata.name,
      namespace: stepAction.metadata.namespace
    }).catch(err => {
  ...
  async function handleDelete() {
    const deletions = toBeDeleted.map(resource =>
      handleDeleteStepAction(resource)
    );
    closeDeleteModal();
  ...
  function openDeleteModal(selectedRows, handleCancelSelection) {
    const resourcesById = keyBy(stepActions, 'metadata.uid');
        ...
        <>
          <Table
            ...
            emptyTextSelectedNamespace={intl.formatMessage(
              ...
              { kind: 'StepActions', selectedNamespace: namespace }
            )}
          />
          {showDeleteModal ? (
            <DeleteModal
